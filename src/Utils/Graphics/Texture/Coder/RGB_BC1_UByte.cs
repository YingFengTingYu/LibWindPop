...
{
    public readonly unsafe struct RGB_BC1_UByte : ITextureCoder, IPitchableTextureCoder, IDirectXTexture, IOpenGLES20CompressedTexture
    {
        public static DXGI_FORMAT DirectXFormat => DXGI_FORMAT.DXGI_FORMAT_BC1_UNORM;

        public static D3DFORMAT Xbox360D3D9Format => new D3DFORMAT(GPUTEXTUREFORMAT.GPUTEXTUREFORMAT_DXT1, GPUENDIAN.GPUENDIAN_NONE, false, GPUSIGN.GPUSIGN_ALL_UNSIGNED, GPUNUMFORMAT.GPUNUMFORMAT_FRACTION, GPUSWIZZLE.GPUSWIZZLE_OBGR);

        public static int OpenGLES20InternalFormat => 0x83F0; // GL_COMPRESSED_RGB_S3TC_DXT1_EXT
        ...
        public readonly void Decode(ReadOnlySpan<byte> srcData, int width, int height, int pitch, RefBitmap dstBitmap)
        {
            ...
            for (int yTile = 0; yTile < height; yTile += 4)
            {
                ...
                {
                    BCCoder.DecodeBC1Block(srcData.Slice(tempDataIndex, 8), decodeBlockData);
                    tempDataIndex += 8;
                    for (int y = 0; y < 4; y++)
                    {
                        if ((yTile + y) >= height)
        ...
        public readonly void Encode(RefBitmap srcBitmap, Span<byte> dstData, int width, int height, int pitch)
        {
            ...
            for (int yTile = 0; yTile < height; yTile += 4)
            {
                ...
                for (int xTile = 0; xTile < width; xTile += 4)
                {
                    ...
                    BCCoder.EncodeBC1Block(dstData.Slice(tempDataIndex, 8), encodeBlockData);
                    tempDataIndex += 8;
                }
            }
        }
