...
{
    public readonly unsafe struct RGB_BC6S_UByte : ITextureCoder, IPitchableTextureCoder, IDirectXTexture, IOpenGLES20CompressedTexture
    {
        public static DXGI_FORMAT DirectXFormat => DXGI_FORMAT.DXGI_FORMAT_BC6H_SF16;

        public static int OpenGLES20InternalFormat => 0x8E8E; // GL_COMPRESSED_RGB_BPTC_SIGNED_FLOAT
        ...
        public readonly void Decode(ReadOnlySpan<byte> srcData, int width, int height, int pitch, RefBitmap dstBitmap)
        {
            ...
            for (int yTile = 0; yTile < height; yTile += 4)
            {
                ...
                {
                    BCCoder.DecodeBC6HSignedBlock(srcData.Slice(tempDataIndex, 16), decodeBlockData);
                    tempDataIndex += 16;
                    for (int y = 0; y < 4; y++)
                    {
                        if ((yTile + y) >= height)
        ...
        public readonly void Encode(RefBitmap srcBitmap, Span<byte> dstData, int width, int height, int pitch)
        {
            ...
            for (int yTile = 0; yTile < height; yTile += 4)
            {
                ...
                for (int xTile = 0; xTile < width; xTile += 4)
                {
                    ...
                    BCCoder.EncodeBC6HSignedBlock(dstData.Slice(tempDataIndex, 16), encodeBlockData);
                    tempDataIndex += 16;
                }
            }
        }
