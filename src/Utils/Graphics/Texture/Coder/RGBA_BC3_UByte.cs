...
{
    public readonly unsafe struct RGBA_BC3_UByte : ITextureCoder, IPitchableTextureCoder, IDirectXTexture, IXbox360D3D9Texture, IOpenGLES20CompressedTexture
    {
        public static DXGI_FORMAT DirectXFormat => DXGI_FORMAT.DXGI_FORMAT_BC3_UNORM;
        ...
        public readonly void Decode(ReadOnlySpan<byte> srcData, int width, int height, int pitch, RefBitmap dstBitmap)
        {
            ...
            for (int yTile = 0; yTile < height; yTile += 4)
            {
                ...
                {
                    BCCoder.DecodeBC3Block(srcData.Slice(tempDataIndex, 16), decodeBlockData);
                    tempDataIndex += 16;
                    for (int y = 0; y < 4; y++)
                    {
                        if ((yTile + y) >= height)
        ...
        public readonly void Encode(RefBitmap srcBitmap, Span<byte> dstData, int width, int height, int pitch)
        {
            ...
            for (int yTile = 0; yTile < height; yTile += 4)
            {
                ...
                for (int xTile = 0; xTile < width; xTile += 4)
                {
                    ...
                    BCCoder.EncodeBC3Block(dstData.Slice(tempDataIndex, 16), encodeBlockData);
                    tempDataIndex += 16;
                }
            }
        }
