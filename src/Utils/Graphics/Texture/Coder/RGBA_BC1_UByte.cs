...
{
    public readonly unsafe struct RGBA_BC1_UByte : ITextureCoder, IPitchableTextureCoder, IDirectXTexture, IXbox360D3D9Texture, IOpenGLES20CompressedTexture
    {
        public static DXGI_FORMAT DirectXFormat => DXGI_FORMAT.DXGI_FORMAT_BC1_UNORM;

        public static D3DFORMAT Xbox360D3D9Format => D3DFORMAT.D3DFMT_LIN_DXT1.GetLEFormat();
        ...
        public readonly void Decode(ReadOnlySpan<byte> srcData, int width, int height, int pitch, RefBitmap dstBitmap)
        {
            ...
            for (int yTile = 0; yTile < height; yTile += 4)
            {
                ...
                {
                    BCCoder.DecodeBC1BlockWithAlpha(srcData.Slice(tempDataIndex, 8), decodeBlockData);
                    tempDataIndex += 8;
                    for (int y = 0; y < 4; y++)
                    {
                        if ((yTile + y) >= height)
        ...
        public readonly void Encode(RefBitmap srcBitmap, Span<byte> dstData, int width, int height, int pitch)
        {
            ...
            for (int yTile = 0; yTile < height; yTile += 4)
            {
                ...
                for (int xTile = 0; xTile < width; xTile += 4)
                {
                    ...
                    BCCoder.EncodeBC1BlockWithAlpha(dstData.Slice(tempDataIndex, 8), encodeBlockData);
                    tempDataIndex += 8;
                }
            }
        }
