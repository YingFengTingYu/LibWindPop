...
{
    public readonly unsafe struct R8_G8_B8_A4_DXT3_UByte : ITextureCoder, IPitchableTextureCoder, IXbox360D3D9Texture, IOpenGLES20CompressedTexture
    {
        ...
        public readonly void Decode(ReadOnlySpan<byte> srcData, int width, int height, int pitch, RefBitmap dstBitmap)
        {
            ...
            for (int yTile = 0; yTile < height; yTile += 4)
            {
                ...
                {
                    BCCoder.DecodeDXT3Block(srcData.Slice(tempDataIndex, 16), decodeBlockData);
                    tempDataIndex += 16;
                    for (int y = 0; y < 4; y++)
                    {
                        if ((yTile + y) >= height)
        ...
        public readonly void Encode(RefBitmap srcBitmap, Span<byte> dstData, int width, int height, int pitch)
        {
            ...
            for (int yTile = 0; yTile < height; yTile += 4)
            {
                ...
                for (int xTile = 0; xTile < width; xTile += 4)
                {
                    ...
                    BCCoder.EncodeDXT3Block(dstData.Slice(tempDataIndex, 16), encodeBlockData);
                    tempDataIndex += 16;
                }
            }
        }
