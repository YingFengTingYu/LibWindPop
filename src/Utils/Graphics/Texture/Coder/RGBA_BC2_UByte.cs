...
{
    public readonly unsafe struct RGBA_BC2_UByte : ITextureCoder, IPitchableTextureCoder, IDirectXTexture, IXbox360D3D9Texture, IOpenGLES20CompressedTexture
    {
        public static DXGI_FORMAT DirectXFormat => DXGI_FORMAT.DXGI_FORMAT_BC2_UNORM;

        public static D3DFORMAT Xbox360D3D9Format => D3DFORMAT.D3DFMT_LIN_DXT3.GetLEFormat();

        public static int OpenGLES20InternalFormat => 0x83F2; // GL_COMPRESSED_RGBA_S3TC_DXT3_EXT
        ...
        public readonly void Decode(ReadOnlySpan<byte> srcData, int width, int height, int pitch, RefBitmap dstBitmap)
        {
            ...
            for (int yTile = 0; yTile < height; yTile += 4)
            {
                ...
                {
                    BCCoder.DecodeBC2Block(srcData.Slice(tempDataIndex, 16), decodeBlockData);
                    tempDataIndex += 16;
                    for (int y = 0; y < 4; y++)
                    {
                        if ((yTile + y) >= height)
        ...
        public readonly void Encode(RefBitmap srcBitmap, Span<byte> dstData, int width, int height, int pitch)
        {
            ...
            for (int yTile = 0; yTile < height; yTile += 4)
            {
                ...
                for (int xTile = 0; xTile < width; xTile += 4)
                {
                    ...
                    BCCoder.EncodeBC2Block(dstData.Slice(tempDataIndex, 16), encodeBlockData);
                    tempDataIndex += 16;
                }
            }
        }
