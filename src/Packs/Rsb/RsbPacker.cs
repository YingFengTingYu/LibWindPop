...
namespace LibWindPop.Packs.Rsb
{
    ...
    {
        public static void Pack(string unpackPath, string rsbPath, IFileSystem fileSystem, ILogger logger, bool throwException)
        {
            ...
            {
                logger.LogError("Composite group info is null", 0, throwException);
            }
            else if (packInfo.Groups == null)
            {
                logger.LogError("Sub group info is null", 0, throwException);
            }
            else
            {
                ...
                using (Stream rsbStream = fileSystem.Create(rsbPath))
                {
                    PackInternal(rsbStream, paths, parentPath, packInfo, fileSystem, logger, encoding, throwException);
                }
                //}
            }
        ...
        {
            if (pack_info.Composites != null && pack_info.Groups != null)
            {
                List<RsbPackPoolInfo> poolList = new List<RsbPackPoolInfo>(512);
                List<RsbPackGroupInfo> groupList = new List<RsbPackGroupInfo>(512);
                if (pack_info.Pools != null)
                {
                    for (int i = 0; i < pack_info.Pools.Length; i++)
                    {
                        poolList.Add(pack_info.Pools[i]);
                    }
                }
                int startPoolCount = poolList.Count;
                for (int i = 0; i < pack_info.Groups.Length; i++)
                {
                    RsbPackGroupInfo? groupInfo = pack_info.Groups[i];
                    if (groupInfo != null)
                    {
                        ...
                        {
                            if (poolList[j].Index == groupInfo.PoolIndex)
                            {
                                match = true;
                                groupInfo.PoolIndex = (uint)j;
                                break;
                            }
                        }
                        if (!match)
                        {
                            groupInfo.PoolIndex = (uint)poolList.Count;
                            poolList.Add(new RsbPackPoolInfo { Name = $"{groupInfo.Name}_AutoPool", Index = uint.MaxValue, BufferCount = 1 });
                        }
                    }
                }
                // Get raw data
                ...
                uint image_count = 0u;
                for (int i = pack_info.Composites.Length - 1; i >= 0; i--)
                {
                    string? id = pack_info.Composites[i].Id;
                    if (id != null)
                    {
                ...
                for (int i = groupList.Count - 1; i >= 0; i--)
                {
                    ...
                    if (pack_info.UseGlobalFileIndexMap)
                    {
                ...
                if (in_mem_len > 0u)
                {
                    using (NativeMemoryOwner raw_data_allocator = new NativeMemoryOwner(in_mem_len))
                    {
                        ...
                        for (int i = 0; i < pack_info.Composites.Length; i++)
                        {
                            string? key = pack_info.Composites[i].Id;
                            if (key != null)
                            {
                        ...
                        // Peek header len
                        uint info_len = pack_info.Version >= 4 ? 0x70u : 0x6Cu;
                        uint resource_map_offset = info_len;
                        uint resource_map_len = pack_info.UseGlobalFileIndexMap ? CompiledMapEncoder.PeekSize((CompiledMapEncodePair*)resource_pair_pointer_number, resource_pair_count) : 0u;
                        ...
                        uint composite_info_count = (uint)pack_info.Composites.Length;
                        uint composite_info_each_len = pack_info.Version >= 3 ? (uint)sizeof(RsbCompositeDescriptorV3V4) : (uint)sizeof(RsbCompositeDescriptorV1);
                        ...
                        uint group_info_each_len = (uint)sizeof(RsbGroupDescriptor);
                        if (pack_info.Version <= 1)
                        {
                        ...
                        uint pool_info_each_len = (uint)sizeof(RsbPoolDescriptor);
                        uint image_info_offset = pool_info_offset + pool_info_count * pool_info_each_len;
                        uint image_info_count = image_count;
                        uint image_info_each_len = pack_info.TextureDescriptorPitch;
                        if (!pack_info.UseGlobalFileIndexMap)
                        {
                        ...
                        if (pack_info.UseManifest)
                        {
                            manifest_info = WindJsonSerializer.TryDeserializeFromFile<RsbManifestInfo>(paths.InfoManifestPath, 0u, fileSystem, logger, false);
                        }
                        Dictionary<string, uint>? manifest_pool_dic = PeekRsbManifestInfo(pack_info.Version, manifest_info, encoding, out manifest_group_info_len, out manifest_resource_info_len, out manifest_pool_len);
                        ...
                        using (IMemoryOwner<byte> rsb_header_allocator = MemoryPool<byte>.Shared.Rent((int)header_len))
                        {
                            ...
                            fixed (byte* rsb_header_allocator_pointer = rsb_header_allocator_span)
                            {
                                ...
                                // Encode info
                                RsbInfo* rsb_info = (RsbInfo*)rsb_header_pointer_number;
                                rsb_info->Magic = 1920164401u;
                                rsb_info->Version = pack_info.Version;
                                rsb_info->MinorVersion = pack_info.MinorVersion;
                                ...
                                rsb_info->GroupDescriptorPitch = group_info_each_len;
                                rsb_info->CompositeCount = composite_info_count;
                                rsb_info->CompositeDescriptorOffset = composite_info_offset;
                                rsb_info->CompositeDescriptorPitch = composite_info_each_len;
                                ...
                                rsb_info->PoolDescriptorPitch = pool_info_each_len;
                                rsb_info->TextureCount = image_info_count;
                                rsb_info->TextureDescriptorOffset = image_info_offset;
                                rsb_info->TextureDescriptorPitch = image_info_each_len;
                                ...
                                if (manifest_info != null && manifest_pool_dic != null)
                                {
                                    PackRsbManifest(pack_info.Version, manifest_info, manifest_pool_dic, encoding, rsb_header_pointer_number + manifest_group_info_offset, rsb_header_pointer_number + manifest_resource_info_offset, rsb_header_pointer_number + manifest_pool_offset);
                                }
                                // Encode cmap
                                if (pack_info.UseGlobalFileIndexMap)
                                {
                                ...
                                if (pack_info.Version >= 3)
                                {
                                    for (uint i = 0u; i < composite_info_count; i++)
                                    {
                                        RsbPackCompositeInfo pack_composite_info = pack_info.Composites[i];
                                        RsbCompositeDescriptorV3V4* composite_info = (RsbCompositeDescriptorV3V4*)current_pointer_number;
                                        RsbSubGroupV3V4* composite_sub_info = (RsbSubGroupV3V4*)(current_pointer_number + 0x80u);
                                        ...
                                        for (uint j = 0u; j < subGroupCount; j++)
                                        {
                                            ...
                                            if (ptr == null)
                                            {
                                                logger.LogWarning($"Can not find group {pack_composite_group_info.Id}", 0);
                                            }
                                            else
                                            {
                                                composite_info->SubGroupCount++;
                                                composite_sub_info->GroupIndex = *(uint*)ptr;
                                                composite_sub_info->Res = pack_composite_group_info.Res;
                                                composite_sub_info->Loc = UInt32StringConvertor.StringToUInt32(pack_composite_group_info.Loc);
                                                composite_sub_info++;
                                            }
                                        }
                                ...
                                else
                                {
                                    ...
                                    {
                                        RsbPackCompositeInfo pack_composite_info = pack_info.Composites[i];
                                        RsbCompositeDescriptorV1* composite_info = (RsbCompositeDescriptorV1*)current_pointer_number;
                                        RsbSubGroupV1* composite_sub_info = (RsbSubGroupV1*)(current_pointer_number + 0x80u);
                                        UnsafeStringHelper.SetUtf16String(pack_composite_info.Name, composite_info->Name, 0x80, encoding);
                                        int subGroupCount = pack_composite_info.SubGroups?.Length ?? 0;
                                        composite_info->SubGroupCount = 0u;
                                        for (uint j = 0u; j < subGroupCount; j++)
                                ...
                                for (uint i = 0u; i < pool_info_count; i++)
                                {
                                    ...
                                    RsbPoolDescriptor* pool_info = (RsbPoolDescriptor*)current_pointer_number;
                                    UnsafeStringHelper.SetUtf16String(pack_pool_info.Name, pool_info->Name, 0x80, encoding);
                                    pool_info->BufferCount = pack_pool_info.BufferCount;
                                ...
                                for (uint i = 0u; i < group_info_count; i++)
                                {
                                    ...
                                    RsbGroupDescriptor* group_info = (RsbGroupDescriptor*)current_pointer_number;
                                    ...
                                    RsgMetadata? rsg_pack_info = WindJsonSerializer.TryDeserializeFromFile<RsgMetadata>(rsg_info_path, 0u, fileSystem, logger, false);
                                    if (rsg_pack_info == null)
                                    {
                                        logger.LogError($"Can not read rsg info at {rsg_info_path}", 0, throwException);
                                    }
                                    else
                                    {
                                        using (Stream rsg_stream = fileSystem.OpenRead(rsg_path))
                                        {
                                            ...
                                            group_info->PoolIndex = pack_group_info.PoolIndex;
                                            ...
                                            RsbPoolDescriptor* pool_info = (RsbPoolDescriptor*)(rsb_header_pointer_number + pool_info_offset + pool_info_each_len * group_info->PoolIndex);
                                            ...
                                            if (pack_group_info.GPUFileList != null)
                                            {
                                                for (int j = 0; j < pack_group_info.GPUFileList.Length; j++)
                                                {
                                                    ...
                                                    RsbTextureDescriptor* image_info = (RsbTextureDescriptor*)current_image_info_pointer_number;
                                                    ...
                                                    {
                                                        image_info->Extend0x10 = pack_image_info.Extend1;
                                                    }
                                                    if (image_info_each_len >= 0x18u)
                                                    {
                                                        image_info->Extend0x14 = pack_image_info.Extend2;
                                                    }
                                ...
                                RsbReverser.ReverseRsbHeader(rsb_info, Endianness.Native, pack_info.UseBigEndian ? Endianness.Big : Endianness.Little);
        ...
        private static unsafe void PackRsbManifest(uint rsbVersion, RsbManifestInfo manifest, Dictionary<string, uint> pool_dic, Encoding encoding, nuint manifest_group_info_pointer_number, nuint manifest_resource_info_pointer_number, nuint manifest_pool_pointer_number)
        {
            ...
            {
                uint manifestSubGroupInfoSize = rsbVersion >= 3u ? (uint)sizeof(ManifestSubGroupInfoForRsbV3V4) : (uint)sizeof(ManifestSubGroupInfoForRsbV1);
                for (int i = 0; i < manifest.CompositeGroups.Length; i++)
                {
                    RsbManifestCompositeInfo composite_info = manifest.CompositeGroups[i];
                    ManifestCompositeGroupInfo* composite = (ManifestCompositeGroupInfo*)manifest_group_info_pointer_number;
                    manifest_group_info_pointer_number += (uint)sizeof(ManifestCompositeGroupInfo);
                    composite->IdOffset = pool_dic[composite_info.Id ?? string.Empty];
                    composite->SubGroupCount = (uint)(composite_info.SubGroups?.Length ?? 0);
                    composite->SubGroupInfoEachSize = manifestSubGroupInfoSize;
                    if (composite_info.SubGroups != null)
                    {
                        for (int j = 0; j < composite_info.SubGroups.Length; j++)
                        {
                            ...
                            if (rsbVersion >= 3u)
                            {
                                ManifestSubGroupInfoForRsbV3V4* group = (ManifestSubGroupInfoForRsbV3V4*)manifest_group_info_pointer_number;
                                group->Res = group_info.Res;
                                group->Loc = UInt32StringConvertor.StringToUInt32(group_info.Loc);
                                group->IdOffset = pool_dic[group_info.Id ?? string.Empty];
                                group->ResourceCount = (uint)(group_info.Resources?.Length ?? 0);
                            }
                            else
                            {
                                ManifestSubGroupInfoForRsbV1* group = (ManifestSubGroupInfoForRsbV1*)manifest_group_info_pointer_number;
                                group->Res = group_info.Res;
                                group->IdOffset = pool_dic[group_info.Id ?? string.Empty];
                                group->ResourceCount = (uint)(group_info.Resources?.Length ?? 0);
                            }
                                ...
                                for (int k = 0; k < group_info.Resources.Length; k++)
                                {
                                    ...
                                    *(uint*)manifest_group_info_pointer_number = current_resource_offset;
                                    ...
                                    else
                                    {
                                        ...
                                        res_img_prop->AtlasFlags = (ushort)resource_info.ImageProperties.AtlasFlags;
                                        res_img_prop->OffsetX = (ushort)resource_info.ImageProperties.OffsetX;
                                        res_img_prop->OffsetY = (ushort)resource_info.ImageProperties.OffsetY;
                                        res_img_prop->AtlasX = (ushort)resource_info.ImageProperties.AtlasX;
                                        res_img_prop->AtlasY = (ushort)resource_info.ImageProperties.AtlasY;
                                        res_img_prop->AtlasWidth = (ushort)resource_info.ImageProperties.AtlasWidth;
                                        res_img_prop->AtlasHeight = (ushort)resource_info.ImageProperties.AtlasHeight;
        ...
        private static unsafe Dictionary<string, uint>? PeekRsbManifestInfo(uint rsbVersion, RsbManifestInfo? manifest, Encoding encoding, out uint manifest_group_info_len, out uint manifest_resource_info_len, out uint manifest_pool_len)
        {
            ...
            {
                RsbManifestCompositeInfo composite_info = manifest.CompositeGroups[i];
                manifest_group_info_len += (uint)sizeof(ManifestCompositeGroupInfo);
