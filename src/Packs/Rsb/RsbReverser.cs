...
namespace LibWindPop.Packs.Rsb
{
    ...
    public static unsafe class RsbReverser
    {
        ...
        public static void ReverseRsbHeader(void* rsb_header_pointer, Endianness in_endian, Endianness out_endian)
        {
            ...
            ResStreamsHeader* rsb_info = (ResStreamsHeader*)rsb_header_pointer;
        ...
        public static void ReverseRsgHeader(void* rsg_header_pointer, Endianness in_endian, Endianness out_endian)
        {
            ...
            ResStreamGroupHeader* rsg_info = (ResStreamGroupHeader*)rsg_header_pointer;
        ...
        private static void ReverseRsgpHeaderEndianAsUnusedEndian(void* header_pointer)
        {
            ...
            ResStreamGroupHeader* rsgp_info = (ResStreamGroupHeader*)header_pointer;
        ...
        {
            ArgumentNullException.ThrowIfNull(header_pointer, nameof(header_pointer));
            ResStreamsHeader* rsb_info = (ResStreamsHeader*)header_pointer;
            ...
            {
                ReverseInfoEndian(header_pointer_number + rsb_info->CompositeDescriptorOffset, rsb_info->CompositeCount, rsb_info->CompositeDescriptorSize, 0x80u);
            }
            ...
            {
                ReverseInfoEndian(header_pointer_number + rsb_info->GroupDescriptorOffset, rsb_info->GroupCount, rsb_info->GroupDescriptorSize, 0x80u);
            }
            // res_streams_pool_info
            if (rsb_info->PoolDescriptorOffset != 0)
            {
                ReverseInfoEndian(header_pointer_number + rsb_info->PoolDescriptorOffset, rsb_info->PoolCount, rsb_info->PoolDescriptorSize, 0x80u);
            }
            // image_info
            if (rsb_info->TextureDescriptorOffset != 0)
            {
                ReverseInfoEndian(header_pointer_number + rsb_info->TextureDescriptorOffset, rsb_info->TextureCount, rsb_info->TextureDescriptorSize, 0u);
            }
            // manifast
            if (rsb_info->ManifestGroupInfoOffset != 0)
            {
                ReverseManifestEndianAsUnusedEndian(rsb_info->MajorVersion, header_pointer_number, header_pointer_number + rsb_info->ManifestGroupInfoOffset, header_pointer_number + rsb_info->ManifestResourceInfoOffset, header_pointer_number + rsb_info->ManifestStringPoolOffset);
            }
            // rsb_info
            ReverseInfoEndian(header_pointer_number, 1u, rsb_info->MajorVersion > 3 ? 0x70u : 0x6Cu, 0u);
        }
        ...
        private static void ReverseRsbHeaderEndianAsNativeEndian(void* header_pointer)
        {
            ResStreamsHeader* rsb_info = (ResStreamsHeader*)header_pointer;
            nuint header_pointer_number = (nuint)header_pointer;
            uint version = BinaryPrimitives.ReverseEndianness(rsb_info->MajorVersion);
            // rsb_info
        ...
        private static void ReverseManifestStructEndian(ManifestGroupInfoV1* ptr)
        {
            ptr->ArtResolution = BinaryPrimitives.ReverseEndianness(ptr->ArtResolution);
            ptr->IdOffset = BinaryPrimitives.ReverseEndianness(ptr->IdOffset);
            ptr->ResourceCount = BinaryPrimitives.ReverseEndianness(ptr->ResourceCount);
        }

        private static void ReverseManifestStructEndian(ManifestGroupInfoV3V4* ptr)
        {
            ptr->ArtResolution = BinaryPrimitives.ReverseEndianness(ptr->ArtResolution);
            ptr->Localization = BinaryPrimitives.ReverseEndianness(ptr->Localization);
            ptr->IdOffset = BinaryPrimitives.ReverseEndianness(ptr->IdOffset);
            ptr->ResourceCount = BinaryPrimitives.ReverseEndianness(ptr->ResourceCount);
        }

        private static void ReverseManifestStructEndian(ManifestCompositeInfo* ptr)
        {
            ptr->IdOffset = BinaryPrimitives.ReverseEndianness(ptr->IdOffset);
            ptr->ChildCount = BinaryPrimitives.ReverseEndianness(ptr->ChildCount);
            ptr->GroupInfoSize = BinaryPrimitives.ReverseEndianness(ptr->GroupInfoSize);
        }
        ...
        private static void ReverseManifestEndianAsUnusedEndian(uint version, nuint header_pointer_number, nuint group_pointer_number, nuint file_pointer_number, nuint pool_pointer_number)
        {
            ...
            ManifestCompositeInfo* composite_pointer;
            ManifestGroupInfoV1* group_pointer_v1 = null;
            ManifestGroupInfoV3V4* group_pointer_v3v4 = null;
            uint* file_pointer;
            ...
            {
                composite_pointer = (ManifestCompositeInfo*)current_group_pointer_number;
                current_group_pointer_number += (uint)sizeof(ManifestCompositeInfo);
                for (uint i = 0u; i < composite_pointer->ChildCount; i++)
                {
                    uint resCount;
                    if (version >= 3u)
                    {
                        group_pointer_v3v4 = (ManifestGroupInfoV3V4*)current_group_pointer_number;
                        resCount = group_pointer_v3v4->ResourceCount;
                    }
                    else
                    {
                        group_pointer_v1 = (ManifestGroupInfoV1*)current_group_pointer_number;
                        resCount = group_pointer_v1->ResourceCount;
                    }
                    current_group_pointer_number += composite_pointer->GroupInfoSize;
        ...
        private static void ReverseManifestEndianAsNativeEndian(uint version, nuint header_pointer_number, nuint group_pointer_number, nuint file_pointer_number, nuint pool_pointer_number)
        {
            nuint current_group_pointer_number = group_pointer_number;
            ManifestCompositeInfo* composite_pointer;
            ManifestGroupInfoV1* group_pointer_v1;
            ManifestGroupInfoV3V4* group_pointer_v3v4;
            uint* file_pointer;
            ManifestResourceHeader* res_header_pointer;
            ManifestResourceImageProperty* image_prop_pointer;
            ManifestResourceUniversalProperty* universal_prop_pointer;
