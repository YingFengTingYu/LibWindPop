...
{
    public static unsafe class A5_Palette_A1_UByte
    {
        public static void Decode(ReadOnlySpan<byte> src_data, int width, int height, RefBitmap dst_bitmap)
        {
            ...
            else
            {
                ...
                {
                    palette[i] = (byte)BitHelper.FiveBitToEightBit(src_data[i + 1]); // In game: 8 * alpha + 1
                }
                DecodeAlpha5PaletteData(alpha_index_data, palette, width, height, dst_bitmap);
            }
        }

        private static void DecodeAlpha5PaletteData(ReadOnlySpan<byte> src_data, ReadOnlySpan<byte> alpha8_palette, int width, int height, RefBitmap dst_bitmap)
        {
        ...
        {
            ThrowHelper.ThrowWhen(width < 0 || height < 0);
            Span<byte> palette = stackalloc byte[32];
            ...
            {
                EncodeAlpha5PaletteData(src_bitmap, alpha_index_data, palette[..paletteCount], width, height);
            }
        }

        private static void EncodeAlpha5PaletteData(RefBitmap src_bitmap, Span<byte> dst_data, ReadOnlySpan<byte> alpha8_palette, int width, int height)
        {
            dst_data.Clear();
            int bit_depth = GetAlphaBit(alpha8_palette.Length);
            if (bit_depth != 0)
            {
                ...
                int bit_counter = bit_depth;
                int index;
                Span<byte> get_index_palette = stackalloc byte[32];
                ...
                for (int y = 0; y < height; y++)
                {
                    ...
                    for (int x = 0; x < width; x++)
                    {
                        index = get_index_palette[BitHelper.EightBitToFiveBit(row[x].Alpha)] & bit_mask;
                        if (bit_counter < bit_depth)
                        {
        ...
        {
            ThrowHelper.ThrowWhen(width < 0 || height < 0);
            Span<bool> palette_buffer = stackalloc bool[32];
            Span<YFColor> row;
            for (int y = 0; y < height; y++)
            {
                row = src_bitmap[y];
                for (int x = 0; x < width; x++)
                {
                    palette_buffer[BitHelper.EightBitToFiveBit(row[x].Alpha)] = true;
                }
            }
            // get palette count
            int palette_count = 0;
            ...
            if ((palette_count == 1 && (palette_buffer[0] || palette_buffer[31])) || (palette_count == 2 && palette_buffer[0] && palette_buffer[31]))
            {
                palette_count = 0;
            }
        ...
        private static void PeekAlphaPalette(RefBitmap src_bitmap, int width, int height, out int palette_count, Span<byte> palette)
        {
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                for (int x = 0; x < width; x++)
                {
                    int alpha_in_palette_value = BitHelper.EightBitToFiveBit(row[x].Alpha);
