        ...
        public void DecodePtx(ReadOnlySpan<byte> ptxData, RefBitmap dstBitmap, uint width, uint height, uint pitch, uint format, uint alphaSize, ILogger logger)
        {
            switch (format)
            {
                ...
                case 23u:
                    ...
                    break;
                case 30u:
                    logger.Log($"{nameof(PtxHandlerPVZ2CNiOSV2)}.{nameof(DecodePtx)} use format {nameof(RGBA_PVRTCI_4BPP_UByte)}", 0);
                ...
                case 31u:
                    ...
                    break;
                case 148u:
                    logger.Log($"{nameof(PtxHandlerPVZ2CNiOSV2)}.{nameof(DecodePtx)} use format {nameof(RGB_PVRTCI_4BPP_UByte)} and {nameof(A8_UByte)}", 0);
        ...
        public void EncodePtx(RefBitmap srcBitmap, Span<byte> ptxData, uint width, uint height, uint pitch, uint format, uint alphaSize, ILogger logger)
        {
            switch (format)
            {
                ...
                case 23u:
                    ...
                    break;
                case 30u:
                    logger.Log($"{nameof(PtxHandlerPVZ2CNiOSV2)}.{nameof(EncodePtx)} use format {nameof(RGBA_PVRTCI_4BPP_UByte)}", 0);
                    TextureCoder.Encode<RGBA_PVRTCI_4BPP_UByte>(srcBitmap, ptxData, (int)width, (int)height);
                    break;
                case 31u:
                    logger.Log($"{nameof(PtxHandlerPVZ2CNiOSV2)}.{nameof(EncodePtx)} use format {nameof(RGBA_PVRTCI_2BPP_UByte)}", 0);
                    TextureCoder.Encode<RGBA_PVRTCI_2BPP_UByte>(srcBitmap, ptxData, (int)width, (int)height);
                    break;
                case 148u:
                    logger.Log($"{nameof(PtxHandlerPVZ2CNiOSV2)}.{nameof(EncodePtx)} use format {nameof(RGB_PVRTCI_4BPP_UByte)} and {nameof(A8_UByte)}", 0);
                    int tex0Size_148 = (int)GetPtxSizeWithoutAlpha(width, height, pitch, format);
